...
Section "Main"
  ...
  ; Install WinPcap
  MessageBox MB_YESNO|MB_ICONQUESTION "BitMeter OS requires the WinPcap library, is it OK to install this?$\r$\n $\r$\n (You can read about WinPcap at http://winpcap.org)" /SD IDYES IDNO noWinpcap
    File ".\WinPcap_4_1_2.exe"
    ExecWait "$INSTDIR\WinPcap_4_1_2.exe"
    Goto endWinpcap
  noWinpcap:
    MessageBox MB_OK|MB_ICONSTOP "You have chosen not to install the WinPcap library, without this component BitMeter OS will not work.$\r$\n $\r$\nThis installer will now terminate, nothing has been installed on your system." 
    Quit 
  endWinpcap:
  ...
  upgradeDb:
    ...
    ExecWait '"$INSTDIR\bmdb.exe" upgrade 8'
    Goto afterDb
  ...
  ExecShell "open" "http://codebox.org.uk/bitmeteros/installed?version=0.8.0&platform=windows"
SectionEnd

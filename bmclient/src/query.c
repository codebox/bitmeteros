...
void doBmClientQuery(){
    ...
    if (prefs.rangeFrom == 0 && prefs.rangeTo == 0){
     // Can't do a query without a range...
        PRINT(COLOUR_RED, "No range has been specified. Use '-r' to specify a range, use '-h' to display help.\n");
     ...
     // Need a valid filter name
        ...
        if (filter == NULL){
            PRINT(COLOUR_RED, "No filter named '%s' could be found.\n", prefs.filter); //TODO put proper msg in #define
         ...
         // First, print out the date/time range covered by the query
            ...
            PRINT(COLOUR_WHITE_2, "From: ");
            PRINT(COLOUR_WHITE_1, "%s %s\n", date1, time1);
            PRINT(COLOUR_WHITE_2, "To:   ");
            PRINT(COLOUR_WHITE_1, "%s %s\n\n", date2, time2);

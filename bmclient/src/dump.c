...
static void printRow(int ignored, struct Data* row){
 ...
 // Format the values
    ...
    if (filter == NULL){
        PRINT(COLOUR_RED, "No filter found for id=%d\n", row->fl);    
        return;
    }
    ...
    if (prefs.dumpFormat == PREF_DUMP_FORMAT_CSV){
     ...
     // For fixed-width output we need to work out how wide to make the columns
        ...
        PRINT(COLOUR_WHITE_1, "%s %s %s ", date, timeFrom, timeTo);
        PRINT(COLOUR_WHITE_2, "%4d ", row->dr);
        PRINT(COLOUR_WHITE_1, "%*s ", vlWidth, vlTxt);
